...
현재 구현 완료된 LLM-facing domain tool은 7개다. `GET_STOCK_NOW_CONTEXT`, `GET_INTRADAY_MOVE_CONTEXT`, `GET_EVENT_TIMELINE_CONTEXT`, `GET_MARKET_INDUSTRY_CONTEXT`, `GET_PRICE_TREND_CONTEXT`, `GET_SUPPLY_DEMAND_CONTEXT`, `GET_FUNDAMENTAL_CONTEXT`는 domain service, domain DTO, `KISTools`의 Spring AI `@Tool` 메서드, 단위 테스트를 기준으로 구현 완료로 본다.
...
| 8 | `COMPARE_STOCKS_CONTEXT` | 미구현 | catalog tool 없음 | 하위 domain tool fan-out/fan-in 기반 복수 종목 비교 DTO 구현 |
| 9 | `GET_MARKET_MOVER_CANDIDATES` | 미구현 | catalog tool 없음 | 거래량, 등락률, 체결강도, 대량체결, 조회상위, 신고/신저 후보 통합 DTO 구현 |
| 10 | `GET_PROGRAM_TRADING_CONTEXT` | 미구현 | catalog tool 없음 | 종목별/시장별 프로그램매매 당일·일별 데이터를 조합한 DTO 구현 |
| 11 | `GET_SHORT_CREDIT_LOAN_CONTEXT` | 미구현 | catalog tool 없음 | 공매도, 신용잔고, 대차거래 추이와 순위 후보를 조합한 리스크 DTO 구현 |
