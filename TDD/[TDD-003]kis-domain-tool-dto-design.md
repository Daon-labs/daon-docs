...
현재 구현 완료된 LLM-facing domain tool은 10개다. `GET_STOCK_NOW_CONTEXT`, `GET_INTRADAY_MOVE_CONTEXT`, `GET_EVENT_TIMELINE_CONTEXT`, `GET_MARKET_INDUSTRY_CONTEXT`, `GET_PRICE_TREND_CONTEXT`, `GET_SUPPLY_DEMAND_CONTEXT`, `GET_FUNDAMENTAL_CONTEXT`, `COMPARE_STOCKS_CONTEXT`, `GET_MARKET_MOVER_CANDIDATES`, `GET_PROGRAM_TRADING_CONTEXT`는 domain service, domain DTO, `KISTools`의 Spring AI `@Tool` 메서드, 단위 테스트를 기준으로 구현 완료로 본다.
